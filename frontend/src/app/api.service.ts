...
import {
  Brand, VehicleModel, VehicleVariant, Dealer, CustomerEnquiry, Lead,
  StatusHistory, LeadNote, CreateEnquiryRequest, UpdateLeadStatusRequest, CreateLeadNoteRequest
} from './models';
...
export class ApiService {
  ...
  dealersForVariant(variantId: string): Observable<Dealer[]> { return this.http.get<Dealer[]>(`/api/vehicles/${variantId}/dealers`); }
  dealers(): Observable<Dealer[]> { return this.http.get<Dealer[]>('/api/dealers'); }

  createEnquiry(request: CreateEnquiryRequest): Observable<CustomerEnquiry> {
    return this.http.post<CustomerEnquiry>('/api/enquiries', request);
  }
  myEnquiries(customerId: string): Observable<CustomerEnquiry[]> {
    const params = new HttpParams().set('customerId', customerId);
    return this.http.get<CustomerEnquiry[]>('/api/enquiries/my', { params });
  }
  enquiry(id: string): Observable<CustomerEnquiry> { return this.http.get<CustomerEnquiry>(`/api/enquiries/${id}`); }

  leads(dealerId: string): Observable<Lead[]> {
  ...
  updateLeadStatus(id: string, request: UpdateLeadStatusRequest): Observable<Lead> {
    return this.http.patch<Lead>(`/api/leads/${id}/status`, request);
  }
  addLeadNote(id: string, request: CreateLeadNoteRequest): Observable<LeadNote> {
    return this.http.post<LeadNote>(`/api/leads/${id}/notes`, request);
  }
